...
def execute_scripts(input):
        t = Timer(lambda: strip_string(input))
        t2 = Timer(lambda: count_spaces(input))
        ...
        space_counts = count_spaces(input)
        puncs_counts = count_punctuations(input)
        sentence_stripped = strip_string(input)
